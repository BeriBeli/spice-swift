...
receive plus one send. One owner configures `SO_NOSIGPIPE`, shutdown unblocks
admitted workers, and raw close occurs only after shutdown completes and every
worker drains, so repeated close and later reuse of the same descriptor number
cannot affect a new owner. The final AGENTS.md simplification removed a
