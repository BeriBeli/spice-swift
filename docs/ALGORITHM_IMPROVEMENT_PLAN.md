...
| AIP-42 | blocked | Replace realtime audio queues with preallocated rings | AIP-00 | Realtime callbacks perform no linear queue movement or per-packet allocation |
| AIP-43 | blocked | Move blocking WebDAV filesystem work to a bounded executor | AIP-00 | Slow I/O does not block an unrelated client while per-client order is preserved |
...
- WebDAV filesystem work runs on a GCD-backed Swift `TaskExecutor` with width
  two, at most 64 pending jobs, and at most 256 MiB of queued retained request
  data. Admission uses checked arithmetic and exposes immutable active, queued,
  retained-byte, completion, failure, cancellation, and rejection diagnostics.
- The server actor performs only bounded parsing, admission, generation checks,
  ...
  requests, and conservative encoded-response reservations share one 256 MiB
  server-retention limit through response-send completion.
- Session handling submits native-backend work without waiting for filesystem or
  response I/O, so the WebDAV channel continues demultiplexing other clients.
  Client close, server close, Session-generation change, and task cancellation
  invalidate queued work; an active blocking syscall may return, but its late
  result must not send a response or revive client state.
...
| --- | --- | --- | --- | --- |
| AIP-00 | 2026-08-28 | PR #33 | Strict Debug and Release `SpiceBenchTests` 16/16; an isolated fake-toolchain execution of the real `run_micro.sh` verifies both `-Xcc` definitions contain literal quote bytes and no backslashes; embedded build-info round-trip, runtime-environment drift, malformed hex/UTF-8/field-count/configuration rejection; Debug and Release product builds; direct no-metadata binary fails before JSON; `sh -n`; `git diff --check` | The Release harness and deterministic micro catalog are implemented, but AIP-00 remains `blocked` until a fresh external live-SPICE artifact is available. Schema 3 binds the reported Swift executable/version/target information, developer directory, toolchain selectors, SDK root/path/version, configuration, and source revision to the measured executable. Runtime `DEVELOPER_DIR`, `TOOLCHAINS`, `SDKROOT`, or Swift-path changes cannot relabel an existing binary. No performance claim is made from the deterministic micro smoke. |
...
| AIP-42 | 2026-08-28 | PR #34 | Full repository strict test gate; full strict `SwiftSpiceTests` 177/177; combined staged-playback, ring, capture, and allocation gate 38 tests / 4 suites in Debug, Release, ThreadSanitizer, and AddressSanitizer with no sanitizer report; strict Debug and Release SwiftSpice builds; `git diff --check`; deterministic ordinary-copy rendering, FIFO publication, overflow/startup atomicity, close and reset during staging, producer serialization, exact 16 MiB queued + 16 MiB playback-staging boundary and one-past rejection, 24,000 one-frame playback packet capacity, 80,000-slot 8 kHz mono/10 s capture case, exact 262,144-slot capture boundary, two-phase capture dequeue lease, entry-snapshot drain bound, 1,024/1,025/2,051-frame capture cases, allocation-free typed failure deferral, multi-callback FIFO, planar Float32 differential conversion, malformed-input recovery, and teardown | Playback and capture keep a 16 MiB logical queue and at most 262,144 metadata slots. Playback additionally preallocates one explicit, equally bounded 16 MiB staging bank, so diagnostics and validation report its true maximum payload ownership as 32 MiB; capture remains single-bank at 16 MiB. An ordinary producer copies into an unpublished free range while render continues consuming old PCM, then publishes only slot metadata under the short gate. Overflow copies into the inactive bank and atomically swaps that bank together with the startup reset, without copying under the gate. Close rejects a late commit, reset rebases it without reviving stale PCM, and serialized producers preserve FIFO. Capture materialization remains outside the producer mutex and each drain is capped at its entry slot snapshot. Oversized callbacks remain bounded and allocation-free; converter calls request no `NSError`, and user-facing failure strings remain on the async drain side. AIP-42's deterministic gates are satisfied, but the item remains `blocked` until AIP-00 supplies an external live-SPICE artifact. No performance claim is made. |
| AIP-43 | 2026-08-28 | PR #35 | Strict Debug build and full repository warnings-as-errors test gate; Public API consumer build; combined executor, WebDAV server, and Session gate 21/21 in Debug, Release, AddressSanitizer, and ThreadSanitizer; `git diff --check`; deterministic width-two/FIFO admission, exact pending and retained-byte boundaries, atomic rejection, cross-client progress, same-client filesystem/send ordering, response-send reservation lifetime, failed-delivery suffix cancellation, generation invalidation during the final transport write, independent inbound/generated-header bounds, close/closeAll same-ID reuse after active-syscall retirement, synchronous compatibility-path retirement rejection and recovery, Session disconnect, late-result suppression, and server reuse | Session WebDAV handling now quick-submits into per-client pumps instead of awaiting filesystem work on the channel event path. A GCD-backed Swift `TaskExecutor` runs at width two; server admission caps 64 total jobs and 256 MiB across incomplete input, parsed requests, and conservative response reservations retained through send. Exact-head reviews found and closed four gaps: a rejected response handoff could continue the same-client suffix, the final awaited transport write needed a second lifecycle check, generated response headers were incorrectly bounded by the inbound request-header setting, and close could reuse a client ID before its unpreemptible synchronous mutation drained. New generations now remain admitted and accounted without occupying an executor permit until the old same-ID operation returns; unrelated clients remain independent. Close, failed delivery, and generation changes suppress queued or late work and release accounting exactly once. The existing public synchronous actor API remains unchanged. Deterministic gates are satisfied, but AIP-43 remains `blocked` until AIP-00 supplies an external live-SPICE artifact; no performance claim is made. |
...
| 2026-08-28 | AIP-00 | Bind source and build-toolchain identity into the Release benchmark executable and expose it as schema-3 `build_metadata` | Repository cleanliness alone cannot prove which already-built binary or compiler produced an artifact. The build script therefore captures the exact Swift path/version/target information and active Xcode/SDK selectors, encodes the snapshot into the C build-info shim together with HEAD, and the CLI reads only that immutable snapshot. This makes later runtime environment drift irrelevant and turns missing or malformed build evidence into a typed pre-output failure. |
| 2026-08-28 | AIP-42 | Process oversized Core Audio tap callbacks through fixed 1,024-frame conversion chunks | `installTap`'s requested buffer size is not a maximum. One preallocated input-format chunk and one ratio-sized output buffer let the serial tap callback copy each interleaved or planar range synchronously, convert it, and enqueue it without allocating callback-owned storage. Whole-callback storage validation prevents malformed later planes from publishing an earlier chunk; timestamps advance by the floor of the input-frame offset in milliseconds and retain UInt32 wrap semantics. |
| 2026-08-28 | AIP-42 | Size capture packet metadata from the output-frame byte budget and defer failure formatting to the drain side | Core Audio does not guarantee a minimum tap callback size, so input-chunk-rate estimates cannot prove that metadata will outlast the payload budget. Every nonempty converted packet contains at least one output frame; reserving one slot per budgeted output frame is therefore a callback-shape-independent upper bound, still capped at 262,144 slots and 16 MiB. Realtime failures retain only a fixed typed token; user-facing `String` construction occurs after the reporting mutex is released on the async drain side. |
| 2026-08-28 | AIP-42 | Bound each capture drain to its entry slot snapshot, omit converter error objects, and stage playback payload before realtime publication | A bounded ring does not by itself bound a drain loop if concurrent production continuously replaces every consumed packet. Snapshotting the entry slot count caps async materialization while preserving FIFO for the next drain. Passing `nil` for the AVAudioConverter error pointer avoids framework-owned error construction on the realtime path; a fixed typed token is sufficient for async reporting. Playback serializes producers and preallocates a second payload bank: ordinary packets use an unpublished free range in the active bank, while replacement packets use the inactive bank. Render therefore continues consuming already-published PCM during payload copy; the short gate only publishes metadata or performs an O(1) bank swap together with the startup reset. The explicit resource contract is 16 MiB queued plus 16 MiB playback staging, with capture retaining its single 16 MiB bank. |
| 2026-08-28 | AIP-43 | Preserve per-client WebDAV order with one owned pump while moving filesystem calls to a width-two bounded Swift task executor | Current spice-gtk `88ad5f1` gives each mux client an independently cancellable asynchronous GIO stream and schedules response reads at idle priority so other streams retain opportunities to progress. Swift keeps parsing, checked admission, and generation state actor-isolated, but a Session submission no longer waits for the native filesystem or response send. Each client completes filesystem work and its response before its next item; unrelated clients overlap through two GCD-backed executor permits. Admission is capped at 64 pending jobs and 256 MiB of queued retained request data. Close invalidates late results, and response I/O occurs after the filesystem permit is released. |
