...
| AIP-43 | 2026-08-28 | PR #35 | Strict Debug build and full repository warnings-as-errors test gate; Public API consumer build; combined executor, WebDAV server, and Session gate 19/19 in Debug, Release, AddressSanitizer, and ThreadSanitizer; `git diff --check`; deterministic width-two/FIFO admission, exact pending and retained-byte boundaries, atomic rejection, cross-client progress, same-client filesystem/send ordering, response-send reservation lifetime, failed-delivery suffix cancellation, generation invalidation during the final transport write, independent inbound/generated-header bounds, client/server close, Session disconnect, late-result suppression, and server reuse | Session WebDAV handling now quick-submits into per-client pumps instead of awaiting filesystem work on the channel event path. A GCD-backed Swift `TaskExecutor` runs at width two; server admission caps 64 total jobs and 256 MiB across incomplete input, parsed requests, and conservative response reservations retained through send. Exact-head review found and closed three gaps: a rejected response handoff could continue the same-client suffix, the final awaited transport write needed a second lifecycle check, and generated response headers were incorrectly bounded by the inbound request-header setting. Close, failed delivery, and generation changes now suppress queued or late work and release accounting exactly once. The existing public synchronous actor API remains unchanged. Deterministic gates are satisfied, but AIP-43 remains `blocked` until AIP-00 supplies an external live-SPICE artifact; no performance claim is made. |
