...
3. **h2c local execution boundary**, split into three independently reviewed
   ...
   - **h2c-3 atomic artifact ownership/index:** one local directory owner must
     bind the exact three canonical records, matching resource sample,
     terminal manifest state, teardown result, reports, and final artifact
     index. A success index may be published only after every referenced object
     is durably present and canonical; partial write, identity mismatch,
     failed teardown, or interrupted publication must fail closed and must not
     be recovered by scanning files or retroactively synthesizing success.
