...
| AIP-43 | 2026-08-28 | PR #35 | Strict Debug build and full repository warnings-as-errors test gate; Public API consumer build; combined executor, WebDAV server, and Session gate 15/15 in Debug, Release, AddressSanitizer, and ThreadSanitizer; `git diff --check`; deterministic width-two/FIFO admission, exact pending and retained-byte boundaries, atomic rejection, cross-client progress, same-client filesystem/send ordering, response-send reservation lifetime, client/server close, Session disconnect, late-result suppression, and server reuse | Session WebDAV handling now quick-submits into per-client pumps instead of awaiting filesystem work on the channel event path. A GCD-backed Swift `TaskExecutor` runs at width two; server admission caps 64 total jobs and 256 MiB across incomplete input, parsed requests, and conservative response reservations retained through send. Close and generation changes suppress late responses and release accounting exactly once. The existing public synchronous actor API remains unchanged. Deterministic gates are satisfied, but AIP-43 remains `blocked` until AIP-00 supplies an external live-SPICE artifact; no performance claim is made. |
