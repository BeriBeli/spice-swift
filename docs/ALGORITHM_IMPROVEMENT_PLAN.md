...
| AIP-42 | 2026-08-28 | PR #34 | Strict Debug and Release combined audio gates 30/30; focused oversized-capture gate 5/5 in Debug, Release, and AddressSanitizer; full strict `SwiftSpiceTests` 169/169; strict SwiftSpice build; `git diff --check`; deterministic 24,000 one-frame playback packet capacity, two-phase capture dequeue lease, overflow/startup gate barrier, allocation-limit boundaries, 1,024/1,025/2,051-frame capture cases, multi-callback FIFO, planar Float32 differential conversion, malformed-input recovery, and close/late-callback teardown | Playback and capture use fixed payload and metadata owners bounded by 16 MiB and 262,144 slots. Playback slot capacity follows its byte budget; capture materialization occurs outside the producer mutex; overflow replacement and startup-gate reset share the `gate → ring` critical section. Capture callbacks larger than the requested tap size are validated once and processed through one preallocated 1,024-frame input chunk plus one preallocated output buffer, preserving plane order, PCM bytes, frame-offset timestamps, and FIFO without callback allocation or linear queue movement. AIP-42's deterministic correctness and allocation gates are satisfied, but the item remains `blocked` rather than `done` until AIP-00 supplies the external live artifact. No performance claim is made. |
