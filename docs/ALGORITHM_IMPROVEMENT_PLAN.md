...
| AIP-11 | 2026-08-26 | PR #21 | `swift test --disable-sandbox -Xswiftc -warnings-as-errors`; `ProcessedSerialBarrierTests` 15/15; combined serial-barrier tests 18/18; AIP-10 batch regression 12/12; `SpiceSessionTests` 59/59; `DisplayChannelTests` 50/50; `git diff --check` | Effective full and implicit-mini serials advance after the physical batch handler and ACK succeed. A SET_ACK main or submessage excludes its complete physical batch from the new ACK window. Handler/transport failure, cancellation, and close terminate only dependent unsatisfied waiters. Superseded receive tasks cannot poison a replacement connection or its shared barrier; an already-started Agent byte stream remains on its captured connection. `migrationRequested` remains recoverable. |
