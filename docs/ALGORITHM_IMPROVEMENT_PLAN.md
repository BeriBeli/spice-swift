...
| AIP-42 | 2026-08-28 | PR #34 | Strict Debug and Release combined audio gates 32/32; focused capture capacity/failure gate 23/23 in Debug, Release, and AddressSanitizer; full strict `SwiftSpiceTests` 171/171; strict SwiftSpice build; `git diff --check`; deterministic 24,000 one-frame playback packet capacity, 80,000-slot 8 kHz mono/10 s capture case, exact 262,144-slot capture boundary and one-past rejection, two-phase capture dequeue lease, overflow/startup gate barrier, allocation-limit boundaries, 1,024/1,025/2,051-frame capture cases, typed failure deferral, multi-callback FIFO, planar Float32 differential conversion, malformed-input recovery, and close/late-callback teardown | Playback and capture use fixed payload and metadata owners bounded by 16 MiB and 262,144 slots. Both paths derive slot capacity from the byte budget and minimum nonempty output frame, so capture metadata remains sufficient for every callback/chunk shape; capture materialization occurs outside the producer mutex; overflow replacement and startup-gate reset share the `gate → ring` critical section. Capture callbacks larger than the requested tap size are validated once and processed through one preallocated 1,024-frame input chunk plus one preallocated output buffer, preserving plane order, PCM bytes, frame-offset timestamps, and FIFO without callback allocation or linear queue movement. Failure callbacks retain typed tokens and defer user-facing string construction until the async drain side. AIP-42's deterministic correctness and allocation gates are satisfied, but the item remains `blocked` rather than `done` until AIP-00 supplies the external live artifact. No performance claim is made. |
...
| 2026-08-28 | AIP-42 | Size capture packet metadata from the output-frame byte budget and defer failure formatting to the drain side | Core Audio does not guarantee a minimum tap callback size, so input-chunk-rate estimates cannot prove that metadata will outlast the payload budget. Every nonempty converted packet contains at least one output frame; reserving one slot per budgeted output frame is therefore a callback-shape-independent upper bound, still capped at 262,144 slots and 16 MiB. Realtime failures retain only a typed token and optional numeric converter code; user-facing `String` construction occurs after the reporting mutex is released on the async drain side. |
