...
invocation sync requests XI2 source rotation. The monitor terminates the old
`xinput` subscription, drains all stdout that source already published through
EOF, and closes the old X connection so unread upstream events are discarded
with that generation. It then establishes the new source endpoint and publishes
a checkpoint behind every old agent call; the worker clears motion ownership
...
an unavailable or malformed monotonic sample invalidates the event.
