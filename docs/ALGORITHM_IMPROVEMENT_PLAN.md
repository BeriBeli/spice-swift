...
| AIP-42 | 2026-08-28 | PR #34 | Strict Debug and Release combined audio gates 34/34; focused capture/gate/drain gate 25/25 in Debug, Release, and AddressSanitizer; full strict `SwiftSpiceTests` 173/173; strict SwiftSpice build; `git diff --check`; deterministic 24,000 one-frame playback packet capacity, 80,000-slot 8 kHz mono/10 s capture case, exact 262,144-slot capture boundary and one-past rejection, two-phase capture dequeue lease, entry-snapshot drain bound under concurrent production, overflow/startup publication barrier with payload copy outside the render gate, allocation-limit boundaries, 1,024/1,025/2,051-frame capture cases, allocation-free typed failure deferral, multi-callback FIFO, planar Float32 differential conversion, malformed-input recovery, and close/late-callback teardown | Playback and capture use fixed payload and metadata owners bounded by 16 MiB and 262,144 slots. Both paths derive slot capacity from the byte budget and minimum nonempty output frame, so capture metadata remains sufficient for every callback/chunk shape. Capture materialization occurs outside the producer mutex and each drain is capped at its entry slot snapshot, so a continuous producer cannot grow one async batch without bound. Playback producers serialize, mark publication pending, copy payload outside the render gate, then atomically commit overflow/startup state in a short gate section; render returns silence while publication is pending instead of waiting on the copy. Capture callbacks larger than the requested tap size are validated once and processed through one preallocated 1,024-frame input chunk plus one preallocated output buffer, preserving plane order, PCM bytes, frame-offset timestamps, and FIFO without callback allocation or linear queue movement. Converter calls request no `NSError`; failure callbacks retain fixed typed tokens and defer user-facing string construction until the async drain side. AIP-42's deterministic correctness and allocation gates are satisfied, but the item remains `blocked` rather than `done` until AIP-00 supplies the external live artifact. No performance claim is made. |
...
| 2026-08-28 | AIP-42 | Bound each capture drain to its entry slot snapshot, omit converter error objects, and separate playback copy from realtime publication | A bounded ring does not by itself bound a drain loop if concurrent production continuously replaces every consumed packet. Snapshotting the entry slot count caps async materialization while preserving FIFO for the next drain. Passing `nil` for the AVAudioConverter error pointer avoids framework-owned error construction on the realtime path; a fixed typed token is sufficient for async reporting. Playback serializes producers and exposes a short `publicationPending` state so payload copy occurs outside the render gate; render emits silence without touching the ring during that interval, and overflow replacement plus startup reset become visible together afterward. |
