...
| AIP-42 | in-progress | Replace realtime audio queues with preallocated rings | — | Realtime callbacks perform no linear queue movement or per-packet allocation |
...
WindowServer occlusion never established real framebuffer demand and teardown
...
click directly through the session after arm, so even a future successful live
run will prove marker-to-exact-frame-to-presented closure, not AppKit input
queue latency. No such live completion is claimed by this implementation-only
slice.
