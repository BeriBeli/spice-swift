...
| AIP-41 | in-progress | Prepare independent Surface snapshots with bounded concurrency | AIP-32 | A blocked Surface does not prevent another from starting and emit order stays stable |
...
| AIP-41 | 2026-08-27 | PR #32 / `9e59a9a` | Apple Silicon SwiftPM CI run `33084567029`, job `98560569588`: build, generated protocol, C-shim analysis, public API, full tests, AddressSanitizer, and coverage passed in 7m31s; full local Debug warnings-as-errors and AddressSanitizer gates; strict Debug and Release builds; `DisplayFramePublisherTests` 23/23; `DisplayChannelTests` 75/75; focused Release and ThreadSanitizer retained-window gates 2 declarations / 3 executions; deterministic reverse-completion gate repeated 100 times / 300 cases; `git diff --check`; exact-head Codex Review found no major issues | Independent Surface snapshots run through a manual Swift task-group window. Production width three matches the three-frame IOSurface pool. Admitted-but-not-emitted work remains within that window; completed results wait only for their ordered prefix, and each frame is actor-validated, emitted, and released before one replacement is admitted. Cancellation releases buffered leases, cancels and drains started children, restores only the never-admitted suffix for a cancelled explicit flush, and never publishes late frames. Tests cover explicit widths three/four, the default width, blocked and reverse-completing Surfaces, per-slot backpressure, stale removal, publisher and parent-task cancellation, and stable publication order. Initial review found a P2 whole-batch retention amplification; the implementation and tests were strengthened, the thread was answered and resolved, and the exact fixed head passed review. No throughput claim is made before AIP-00. |
