...
- GLZ keeps dictionary coordination in an actor, bounds the parsed program,
  ...
  to bounded workers. Color overlap copies preserve long-distance phase while
  growing an initialized prefix instead of issuing one copy per short period.
...
| AIP-32 | 2026-08-27 | PR #29 / `b2bca56` | Apple Silicon SwiftPM CI run `33066140130`, job `98496548673`: build, generated protocol, C-shim analysis, public API, full tests, AddressSanitizer, and coverage passed; full local Debug/Release, ThreadSanitizer, and AddressSanitizer gates; `SpiceCodecsTests` 59/59; `DisplayChannelTests` 73/73; `git diff --check`; Codex Review of `b2bca56` reported no major issues; all three P1 review threads resolved | One Session-owned FIFO GCD-backed Swift `TaskExecutor` runs stateless JPEG/LZ/QUIC/palette/ZLIB and independent GLZ CPU work at width two, with 64 pending jobs and 256 MiB queued-retention limits; MJPEG remains per-stream serial. GLZ parses at most 1,048,576 operations, waits for immutable dependency snapshots outside worker permits, reserves image IDs, rejects stale clear generations, commits out of order while advancing one deterministic contiguous eviction tail, and checks cancellation inside large copies. Review found and closed three amplification/complexity gaps: dictionary and executor waits now conservatively charge parsed programs and dependency snapshots; distance-one RGB/RGB16 overlap uses 15 prefix-doubling chunks for 16,383 pixels instead of 16,383 `memmove` calls while the 65,537-distance phase fixture stays bit-exact; and Display executor admission charges the full physical message owner for generic codecs, ZLIB_GLZ, and palette work, with exact-limit and one-byte-under rejection tests. Swift 6.3 typed-continuation and `withUnsafeBytes` compatibility is covered by CI. No throughput claim is made before AIP-00. |
